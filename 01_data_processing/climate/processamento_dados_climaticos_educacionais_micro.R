...
# Carregar os dados
weather <- fread("./output/weather data/final_weather_data_1940_2020_micro.csv")
censo_escolar <- fread("./output/Censo_da_educacao_basica_micro.csv")
censo_ensino_superior <- fread("./output/Censo_do_ensino_superior_micro.csv")

#
weather <- weather[,municipality_code:= NULL]
...
merged_data <- merge(weather, censo_escolar, by = c("cod_rgi", "year"), all.x = TRUE)
merged_data <- merge(merged_data, censo_ensino_superior, by = c("cod_rgi", "year"), all.x = TRUE)
...
fwrite(merged_data, "./output/weather data/merged_climate_education_data_micro.csv")
