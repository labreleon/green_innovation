...
mun <- read_immediate_region(code_immediate = "all", year = 2020)
...
mun <- mun[, .(code_immediate, code_state)]
mun[, `:=` (latitude = coords_centroide$Y, longitude = coords_centroide$X)]
mun$code_immediate <- as.character(mun$code_immediate)
...
mun <- merge(mun, micro, by.x = "code_immediate", by.y = "cod_rgi", all.x = TRUE)

setnames(mun,c("code_immediate","CD_GEOCODI"),c("cod_rgi","code_muni"))
...
fwrite(mun, "./output/dummy_micro_year_state.csv", row.names = FALSE)
