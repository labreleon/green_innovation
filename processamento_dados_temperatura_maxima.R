...
processar_arquivo <- function(arquivo) {
  ...
  media_anual <- dados[, .(media_anual_max_temp = mean(max_temperature, na.rm = TRUE)), by = .(municipality_code, year)]
...
media_historica_sd <- resultados[year < 2000, .(media_historica = mean(media_anual_max_temp, na.rm = TRUE), sd = sd(media_anual_max_temp, na.rm = TRUE)), by = municipality_code]
...
#mean
dados_finais[year > 1999 & year < 2011, mean_temperature_00_10 := mean(media_anual_max_temp),by = municipality_code]
dados_finais[year > 1999 & year < 2021, mean_temperature_00_20 := mean(media_anual_max_temp),by = municipality_code]



# Variável contínua
dados_finais[, cont_shock_temperature := (media_anual_max_temp - media_historica) / sd]
dados_finais[, cont_shock_temperature_long_00_10 := (mean_temperature_00_10  - media_historica) / sd]
dados_finais[, cont_shock_temperature_long_00_20 := (mean_temperature_00_20  - media_historica) / sd]
...
dados_finais[, moving_average_shock_temperature_ten_year := frollmean(cont_shock_temperature, period, align = "right")]

# Renomear a coluna dummy para dummy_temperature
setnames(dados_finais, "dummy", "dummy_temperature")

# Selecionar as colunas finais
dados_finais <- dados_finais[, .(municipality_code, year, media_anual_max_temp, dummy_temperature, cont_shock_temperature, moving_average_shock_temperature_ten_year,cont_shock_temperature_long_00_10,cont_shock_temperature_long_00_20)]
...
fwrite(dados_finais, "C:/Users/User/OneDrive - Fundacao Getulio Vargas - FGV/Green innovation/output/weather data/final_temperature_data_1940_2020.csv")