...
extract_or_na <- function(result_list, i, field) {
  if (length(result_list) < i || is.null(result_list[[i]])) {
    return(NA_real_)
  }
  result_list[[i]][[field]]
}
...
for(i in 1:length(dep_vars)) {
  ...
  cat("  Temperature coef:   ", sprintf("%.4f", extract_or_na(results_mun_year_trend, i, "coef_temp")), "\n")
  cat("  Temperature SE:     ", sprintf("%.4f", extract_or_na(results_mun_year_trend, i, "se_temp")), "\n")
  cat("  Temperature p-value:", sprintf("%.4f", extract_or_na(results_mun_year_trend, i, "pval_temp")), "\n")
  cat("  Precipitation coef: ", sprintf("%.4f", extract_or_na(results_mun_year_trend, i, "coef_precip")), "\n")
  cat("  Precipitation SE:   ", sprintf("%.4f", extract_or_na(results_mun_year_trend, i, "se_precip")), "\n")
  cat("  Precipitation p-val:", sprintf("%.4f", extract_or_na(results_mun_year_trend, i, "pval_precip")), "\n")
  cat("  State-Year Trend:   ", sprintf("%.4f", extract_or_na(results_mun_year_trend, i, "coef_trend")), "\n")
  cat("  Trend SE:           ", sprintf("%.4f", extract_or_na(results_mun_year_trend, i, "se_trend")), "\n")
  cat("  Observations:       ", extract_or_na(results_mun_year_trend, i, "n_obs"), "\n")
  cat("  R²:                 ", sprintf("%.3f", extract_or_na(results_mun_year_trend, i, "r2")), "\n\n")
}
...
latex_comparison_hac <- c(
  ...
  "\\resizebox{\\textwidth}{!}{%",
  "\\begin{threeparttable}",
  "\\caption{Weather Shocks and Firms: Spatial + Serial HAC}",
  "\\label{tab:firms_comparison_hac}",
  "\\begin{tabular}{lcccccc}",
  "\\toprule",
  "\\multicolumn{7}{c}{\\textbf{Weather Shocks and Active Firms (2000-2020)}}\\\\",
  "\\midrule",
...
output_file_hac <- "table_firms_municipal_cluster_comparison_hac.tex"
...
temp_se_row <- ""
for(i in 1:3) {
  coef_val <- extract_or_na(results_mun_year_trend, i, "coef_temp")
  se_val <- extract_or_na(results_mun_year_trend, i, "se_temp")
  pval <- extract_or_na(results_mun_year_trend, i, "pval_temp")
  stars <- add_stars(pval)
  digits <- ifelse(abs(coef_val) < 0.01, 4, 3)
  temp_row <- paste0(temp_row, "  &   ", format_coef(coef_val, digits), stars)
  temp_se_row <- paste0(temp_se_row, "  & (", format_coef(se_val, digits), ")")
...
precip_se_row <- ""
for(i in 1:3) {
  coef_val <- extract_or_na(results_mun_year_trend, i, "coef_precip")
  se_val <- extract_or_na(results_mun_year_trend, i, "se_precip")
  pval <- extract_or_na(results_mun_year_trend, i, "pval_precip")
  stars <- add_stars(pval)
  digits <- ifelse(abs(coef_val) < 0.01, 4, 3)
  precip_row <- paste0(precip_row, "  &   ", format_coef(coef_val, digits), stars)
  precip_se_row <- paste0(precip_se_row, "  & (", format_coef(se_val, digits), ")")
...
trend_se_row <- ""
for(i in 1:3) {
  coef_val <- extract_or_na(results_mun_year_trend, i, "coef_trend")
  se_val <- extract_or_na(results_mun_year_trend, i, "se_trend")
  pval <- extract_or_na(results_mun_year_trend, i, "pval_trend")
  stars <- add_stars(pval)
  digits <- ifelse(abs(coef_val) < 0.01, 4, 3)
...
latex_mun_year_trend <- c(
  ...
  "\\midrule",
  paste0("Observations        & ",
         format(extract_or_na(results_mun_year_trend, 1, "n_obs"), big.mark = ","), " &",
         format(extract_or_na(results_mun_year_trend, 2, "n_obs"), big.mark = ","), " &",
         format(extract_or_na(results_mun_year_trend, 3, "n_obs"), big.mark = ","), " \\\\"),
  paste0("$R^2$                &  ",
         format_coef(extract_or_na(results_mun_year_trend, 1, "r2"), 3), " & ",
         format_coef(extract_or_na(results_mun_year_trend, 2, "r2"), 3), " & ",
         format_coef(extract_or_na(results_mun_year_trend, 3, "r2"), 3), " \\\\"),
  paste0("Adjusted $R^2$       &  ",
         format_coef(extract_or_na(results_mun_year_trend, 1, "r2_adj"), 3), " & ",
         format_coef(extract_or_na(results_mun_year_trend, 2, "r2_adj"), 3), " & ",
         format_coef(extract_or_na(results_mun_year_trend, 3, "r2_adj"), 3), " \\\\"),
...
latex_mun_year_trend_hac <- c(
  ...
  "\\resizebox{0.75\\textwidth}{!}{%",
  "\\begin{threeparttable}",
  "\\caption{Municipality-Year Fixed Effects with State-Year Trends - Firms (HAC)}",
  "\\label{tab:firms_mun_year_trend_hac}",
  "\\begin{tabular}{lccc}",
  "\\toprule",
  "\\multicolumn{4}{c}{\\textbf{Weather Shocks and Firms - Municipality×Year FE + State Trends}}\\\\",
  "\\midrule",
...
output_file_mun_year_trend_hac <- "table_firms_municipal_cluster_mun_year_trend_hac.tex"
...
for(i in 1:3) {
  ...
  cat("  Temperature (β):    ", format_coef(extract_or_na(results_mun_year_trend, i, "coef_temp"), 4),
      " (", format_coef(extract_or_na(results_mun_year_trend, i, "se_temp"), 4), ")",
      add_stars(extract_or_na(results_mun_year_trend, i, "pval_temp")), "\n", sep = "")
  cat("  Precipitation (η):  ", format_coef(extract_or_na(results_mun_year_trend, i, "coef_precip"), 4),
      " (", format_coef(extract_or_na(results_mun_year_trend, i, "se_precip"), 4), ")",
      add_stars(extract_or_na(results_mun_year_trend, i, "pval_precip")), "\n", sep = "")
  cat("  State-Year Trend:   ", format_coef(extract_or_na(results_mun_year_trend, i, "coef_trend"), 4),
      " (", format_coef(extract_or_na(results_mun_year_trend, i, "se_trend"), 4), ")",
      add_stars(extract_or_na(results_mun_year_trend, i, "pval_trend")), "\n", sep = "")
  ...
  cat("  Observations:       ", extract_or_na(results_mun_year_trend, i, "n_obs"), "\n", sep = "")
  cat("  R²:                 ", format_coef(extract_or_na(results_mun_year_trend, i, "r2"), 3), "\n\n", sep = "")
}
