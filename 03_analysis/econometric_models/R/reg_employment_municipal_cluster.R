...
for(i in 1:length(dep_vars)) {
  ...
  # Print mun-year FE + trend results
...
# -------------------------------------------------------------------------
# TABLE 4: Municipality-Year Fixed Effects + State-Year Trends
# -------------------------------------------------------------------------
...
latex_mun_year_trend <- c(
  ...
  "\\resizebox{0.75\\textwidth}{!}{%",
  "\\begin{threeparttable}",
  "\\caption{Municipality-Year Fixed Effects with State-Year Trends}",
  "\\label{tab:employment_mun_year_trend}",
  "\\begin{tabular}{lccc}",
  "\\toprule",
...
# Write table 4
output_file_mun_year_trend <- "table_employment_municipal_cluster_mun_year_trend.tex"
writeLines(latex_mun_year_trend, output_file_mun_year_trend)
cat("Table 4 saved to:", output_file_mun_year_trend, "\n")
