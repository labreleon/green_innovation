...
cat("LAGGED OUTCOME PLACEBO TEST - PATENT OUTCOMES\n")
...
if(!"prop_verde" %in% names(data) && "qtd_pat" %in% names(data) && "qtd_pat_verde" %in% names(data)) {
  data[, prop_verde := ifelse(qtd_pat > 0, qtd_pat_verde / qtd_pat, 0)]
}
...
cat("\n\n")
cat("=" %>% rep(70) %>% paste(collapse = ""), "\n")
cat("SUMMARY: LAGGED OUTCOME PLACEBO TEST - PATENTS\n")
cat("=" %>% rep(70) %>% paste(collapse = ""), "\n\n")
...
if (n_sig_temp_bonf == 0 && n_sig_precip_bonf == 0) {
  ...
  cat("   Current climate shocks do NOT predict past patent outcomes.\n")
  ...
  cat("     - Correlated trends between climate and patents\n")
...
# Save results
save(all_results, results_table,
     file = "./output/placebo_tests/patents_lagged_outcome_placebo.RData")

# Save CSV for easy viewing
write.csv(results_table,
          file = "./output/placebo_tests/patents_lagged_outcome_placebo.csv",
          row.names = FALSE)

cat("\nResults saved to:\n")
cat("  - ./output/placebo_tests/patents_lagged_outcome_placebo.RData\n")
cat("  - ./output/placebo_tests/patents_lagged_outcome_placebo.csv\n")
