...
vars_to_demean <- c(dep_vars, "cont_shock_temp", "year_state_trend")
...
for(i in 1:length(dep_vars)) {
  ...
  formula_basic <- as.formula(paste0(dv, " ~ cont_shock_temp | mun_code + year"))
  ...
  formula_state_year <- as.formula(paste0(dv, " ~ cont_shock_temp | mun_code + state_year"))
  ...
  formula_mun_year_trend <- as.formula(paste0(dv, " ~ cont_shock_temp + year_state_trend | mun_code + year"))
  ...
  tryCatch({
    ...
    hac_results <- compute_spatial_hac_vcov(
      data = data_model_df,
      y_var = paste0(dv, "_dm"),
      x_vars = c("cont_shock_temp_dm", "year_state_trend_dm"),
      lat_var = "lat",
      lon_var = "lon",
      time_var = "year",
      panel_var = "mun_code",
