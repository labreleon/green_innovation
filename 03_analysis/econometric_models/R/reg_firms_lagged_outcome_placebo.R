# ============================================================================
# LAGGED OUTCOME PLACEBO TEST - FIRM OUTCOMES
...
cat("LAGGED OUTCOME PLACEBO TEST - FIRM OUTCOMES\n")
...
if(!"prop_verde" %in% names(data) && "firmas_ativas" %in% names(data) && "firmas_verde" %in% names(data)) {
  data[, prop_verde := ifelse(firmas_ativas > 0, firmas_verde / firmas_ativas, 0)]
}
...
cat("\n\n")
cat("=" %>% rep(70) %>% paste(collapse = ""), "\n")
cat("SUMMARY: LAGGED OUTCOME PLACEBO TEST - FIRMS\n")
cat("=" %>% rep(70) %>% paste(collapse = ""), "\n\n")
...
if (n_sig_temp_bonf == 0 && n_sig_precip_bonf == 0) {
  ...
  cat("   Current climate shocks do NOT predict past firm outcomes.\n")
  ...
  cat("     - Correlated trends between climate and firm formation\n")
...
save(all_results, results_table, results_table_trend,
     file = "./output/placebo_tests/firms_lagged_outcome_placebo.RData")

# Save CSV for easy viewing
write.csv(results_table,
          file = "./output/placebo_tests/firms_lagged_outcome_placebo.csv",
          row.names = FALSE)

if (nrow(results_table_trend) > 0) {
  write.csv(results_table_trend,
            file = "./output/placebo_tests/firms_lagged_outcome_placebo_with_trend.csv",
            row.names = FALSE)
}

cat("\nResults saved to:\n")
cat("  - ./output/placebo_tests/firms_lagged_outcome_placebo.RData\n")
cat("  - ./output/placebo_tests/firms_lagged_outcome_placebo.csv\n")
cat("  - ./output/placebo_tests/firms_lagged_outcome_placebo_with_trend.csv\n")
