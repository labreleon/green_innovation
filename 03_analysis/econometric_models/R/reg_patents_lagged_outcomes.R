# ============================================================================
# LAGGED OUTCOMES ANALYSIS - PATENTS (2000-2020)
# Regressoes com outcomes defasados: Y_{t+k} = beta * Shock_t + alpha_i + gamma_t + e
# Replicacao completa da estrutura de reg_patents_municipal_cluster.R
# ============================================================================
...
cat("=" , rep("=", 79), "\n", sep = "")
cat("LAGGED OUTCOMES ANALYSIS - PATENTS\n")
  ...
  for(i in 1:length(dep_vars)) {
    ...
    model_basic <- tryCatch({
      feols(formula_basic, data = data_lag, cluster = ~mun_code,
            weights = resolve_weight_vector(data_lag))
    }, error = function(e) {
...
for(model_type in c("basic", "state_year", "mun_year_trend")) {
  ...
  for(i in 1:length(dep_vars)) {
    ...
    latex_table <- c(
      ...
      paste0("\\label{tab:patents_lagged_", dv, "_", model_suffix, "}"),
    ...
    output_file <- paste0("table_patents_lagged_outcomes_", dv, "_", model_suffix, ".tex")
...
latex_comparison <- c(
  ...
  "\\resizebox{\\textwidth}{!}{%",
  "\\begin{threeparttable}",
  "\\caption{Lagged Patent Outcomes: Effect of Current Shocks on Future Outcomes}",
  "\\label{tab:patents_lagged_comparison}",
...
latex_comparison <- c(
  ...
  "\\begin{tablenotes}",
  "\\footnotesize",
  "\\item \\textit{Note:} This table shows the effect of climate shocks at time $t$ on patent outcomes at time $t+k$. ",
...
output_file_comparison <- "table_patents_lagged_outcomes_comparison.tex"
...
cat("\n", rep("=", 80), "\n", sep = "")
cat("RESULTS SUMMARY - LAGGED PATENT OUTCOMES\n")
cat(rep("=", 80), "\n\n", sep = "")
