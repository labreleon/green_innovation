...
# Based on reg_employment_shortrun_with_state_trend.R specifications
# ============================================================================
#
...
base_dep_vars <- c("total_jobs", "green_jobs", "prop_verde")
base_dep_labels <- c("Total Jobs", "Green Jobs", "Prop. Green Jobs")
...
for (lag in OUTCOME_LAGS) {
  ...
  for (i in seq_along(dep_vars)) {
    ...
    # -----------------------------------------------------------------------
    formula_state_trend <- as.formula(paste0(dv, " ~ cont_shock_temp + cont_shock_precip + year_state_trend | mun_code + year"))
    ...
    results_clustered <- rbind(results_clustered, data.frame(
      ...
      model = "state_trend",
      se_type = "clustered",
      coef_temp = coef(model_state_trend)["cont_shock_temp"],
      se_temp = se(model_state_trend)["cont_shock_temp"],
      pval_temp = pvalue(model_state_trend)["cont_shock_temp"],
      coef_precip = coef(model_state_trend)["cont_shock_precip"],
      se_precip = se(model_state_trend)["cont_shock_precip"],
      pval_precip = pvalue(model_state_trend)["cont_shock_precip"],
      coef_trend = coef(model_state_trend)["year_state_trend"],
      se_trend = se(model_state_trend)["year_state_trend"],
      pval_trend = pvalue(model_state_trend)["year_state_trend"],
      n_obs = nobs(model_state_trend),
      r2 = r2(model_state_trend, type = "r2"),
      r2_adj = r2(model_state_trend, type = "ar2")
    ))
...
save(results_clustered, results_hac,
     file = "./output/placebo_tests/employment_lagged_outcome_placebo_with_state_trend.RData")

write.csv(results_clustered,
          file = "./output/placebo_tests/employment_lagged_outcome_placebo_with_state_trend_clustered.csv",
          row.names = FALSE)

write.csv(results_hac,
          file = "./output/placebo_tests/employment_lagged_outcome_placebo_with_state_trend_hac.csv",
          row.names = FALSE)

cat("\nResults saved to:\n")
cat("  - ./output/placebo_tests/employment_lagged_outcome_placebo_with_state_trend.RData\n")
cat("  - ./output/placebo_tests/employment_lagged_outcome_placebo_with_state_trend_clustered.csv\n")
cat("  - ./output/placebo_tests/employment_lagged_outcome_placebo_with_state_trend_hac.csv\n")
cat("\nScript completed.\n")
